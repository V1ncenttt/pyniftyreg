...
if __name__ == '__main__':
    ...
    for i in range(len(ids)):
        ...
        img_moving_f3d_back = f'output_{id}0/updated_resampled_seg_f3d_back_{id}0.nii.gz'
        ...
        # Store results
        results.append([id, dice_front_aladin, dice_front_f3d, dice_back_aladin, dice_back_f3d])

    # Create a DataFrame
    df = pd.DataFrame(results, columns=['ID', 'Dice Front Aladin', 'Dice Front F3D', 'Dice Back Aladin', 'Dice Back F3D'])
    ...
    front_se = df[['Dice Front Aladin', 'Dice Front F3D']].stack().std() / np.sqrt(len(df) * 2)
    back_se = df[['Dice Back Aladin', 'Dice Back F3D']].stack().std() / np.sqrt(len(df) * 2)
    ...
    # Front Dice CI
    plt.errorbar(['Front'], [front_mean], yerr=[front_margin_of_error], fmt='o', label='Front Dice', capsize=5)

    # Back Dice CI
    plt.errorbar(['Back'], [back_mean], yerr=[back_margin_of_error], fmt='o', label='Back Dice', capsize=5)
