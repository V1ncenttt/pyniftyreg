...
class AirwaySegmentRematcher:
    ...
    def centroid_based_matching(self, baseline, followup) -> np.ndarray:
        ...
        matches = {i:None for i in range(1, max(max(baseline_labels), max(followup_labels)) + 1)}
    ...
    def rematch(self, baseline, followup) -> np.ndarray:
        ...
        print('3/3 : Saving the rematched segmentation to rematched_segmentation.nii.gz')
